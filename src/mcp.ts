...
export class CloudflareAdminMCP extends McpAgent<Env> {
  server = new McpServer({ name: "cloudflare-admin", version: "1.0.0" });
  ...
  async init() {
    ...
    this.server.tool(
      ...
      async ({ zone_id, record_id }) =>
        ok(await deleteDnsRecord(token(), zone_id, record_id))
    );
